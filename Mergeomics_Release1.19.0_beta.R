...
depth=1) {
    ...
    noddata$KD_MODULE <- vapply(noddata$NODE, function(x){
        if(x %in% drivers$NODNAMES){
            return(do.call("paste",
                           c(as.list(drivers$MODNAMES[drivers$NODNAMES==x]), 
                                     list("sep"=","))))
        } else{
            return("Not a KD")
        }
    ...
    noddata$KD_TOP_MODULE <- vapply(noddata$NODE, function(x){
        if(x %in% drivers$NODNAMES){
            ...
            return(kdres$MODNAMES[which(min(kdres$FDR)==kdres$FDR)])
        } else{
            return("Not a KD")
        }
    }, "character")
    noddata$KD_BORDER_COLOR <- vapply(noddata$NODE, function(x){
        if(x %in% drivers$NODNAMES){
            # find top module
            kdres <- drivers[drivers$NODNAMES==x,]
            return(kdres$COLOR[which(min(kdres$FDR)==kdres$FDR)])
        } else{
            return("Not a KD")
        }
    }, "character")
    ...
    if (!is.null(noddata)) tool.save(frame=noddata[, c("NODE", "LABEL", "COLOR", "SIZE", "SHAPE", 
                                                       "SECTOR","URL", "MODULE","KD_MODULE","KD_TOP_MODULE",
                                                       "KD_BORDER_COLOR","LABELSIZE")], file=nodfile)
...
ssea.start.configure <- function(plan) {
    ...
    plan$modfile == "undefined"){
        ...
        cat("\nParameters:\n")
        if(plan$permtype=="locus"){
            cat("  Permutation type: marker\n", sep="")
        }
        else{
